...
public class UsedCardInReader extends TestState {
    public UsedCardInReader() {
    }
    ...
    public IEventEnum onEventPreHandle(TestStateDataObject stateDataObject) {
        ...
        if (message.getDescriptor().getMsgClass() == MessageClass.Generic &&
            ...
            if (resp.getResponse() == GenCmdRspCard.MessageCodeType.RemoveCard) {
    ...
    private boolean sendMessageToReader(TestStateDataObject stateDataObject) {
        GenCmdReqCard req = new GenCmdReqCard(GenCmdReqCard.MessageCodeType.RemoveCard);

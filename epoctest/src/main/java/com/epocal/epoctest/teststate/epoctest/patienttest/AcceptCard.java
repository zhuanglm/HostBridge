...
import com.epocal.reader.nextgen.message.request.generic.command.GenCmdReqCard;
import com.epocal.reader.nextgen.message.response.generic.command.GenCmdRspCard;
...
public class AcceptCard extends TestState {
    public AcceptCard() {
    }
    ...
    public IEventEnum onEventPreHandle(TestStateDataObject stateDataObject) {
        ...
        if (message.getDescriptor().getMsgClass() == MessageClass.Generic &&
                ...
                message.getDescriptor().getType() == GenericCommandResponse.CardResponse.value) {
            stateDataObject.StopTimer();
            GenCmdRspCard resp = (GenCmdRspCard) message;
            if (resp == null) {
                TestEventInfo eventInfo = new TestEventInfo();
                eventInfo.setTestEventInfoType(TestEventInfoType.ERROR_INFO);
                eventInfo.setTestStatusErrorType(TestStatusErrorType.COMMUNICATIONFAILED);
            ...
            if (resp.getMessageCode() == GenCmdRspCard.MessageCodeType.AcceptCard.value) {
                if (resp.getResponseDetails() == GenCmdRspCard.DataResponseCode.ACK.value) {
                    return TestStateEventEnum.GetDeviceStatusAfterTestStart;
                }
            }
            TestEventInfo eventInfo = new TestEventInfo();
            eventInfo.setTestEventInfoType(TestEventInfoType.ERROR_INFO);
    ...
    private boolean sendMessageToReader(TestStateDataObject stateDataObject) {
        GenCmdReqCard req = new GenCmdReqCard(GenCmdReqCard.MessageCodeType.AcceptCard);

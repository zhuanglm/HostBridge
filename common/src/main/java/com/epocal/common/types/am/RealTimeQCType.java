...
public enum RealTimeQCType {
    ...
    RealTimeQCType(int enumIndex) {
        // Get the actual enum value from the C++ enum via the JNIEnumResolverService.channelTypeValue() call
        this.value = JNIEnumValueResolverService.RealTimeQCTypeValue(enumIndex);
     ...
     */
    public static RealTimeQCType convert(int value) {
        int enumIndex = Integer.MAX_VALUE;

        for (RealTimeQCType rc : RealTimeQCType.values()) {
            if (rc.value == value) {
                enumIndex = rc.index;
                break;
            }
        ...
        if (enumIndex == Integer.MAX_VALUE) {
            return RealTimeQCType.ENUM_UNINITIALIZED;
        }

        return RealTimeQCType.values()[enumIndex];
    }

    public static RealTimeQCType fromInt(int i) {
        return convert(i);
    }

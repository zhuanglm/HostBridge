...
public enum Temperatures {
     ...
     */
    public static Temperatures convert(int value) {
        int enumIndex = Integer.MAX_VALUE;

        for (Temperatures rc : Temperatures.values()) {
            if (rc.value == value) {
                enumIndex = rc.index;
                break;
            }
        ...
        if (enumIndex == Integer.MAX_VALUE) {
            return Temperatures.ENUM_UNINITIALIZED;
        }

        return Temperatures.values()[enumIndex];
    }
    public int getValue() {
        return value;
    }

    public static Temperatures fromInt(int i) {
        return convert(i);
    }

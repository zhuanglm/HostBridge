...
public enum AgeCategory {
     ...
     */
    public static AgeCategory convert(int value) {
        int enumIndex = Integer.MAX_VALUE;

        for (AgeCategory rc : AgeCategory.values()) {
            if (rc.value == value) {
                enumIndex = rc.index;
                break;
            }
        ...
        if (enumIndex == Integer.MAX_VALUE) {
            return AgeCategory.ENUM_UNINITIALIZED;
        }

        return AgeCategory.values()[enumIndex];
    }

    public static AgeCategory fromInt(int i) {
        return convert(i);
    }

...
public enum AGAPType {
     ...
     */
    public static AGAPType convert(int value) {
        int enumIndex = Integer.MAX_VALUE;

        for (AGAPType rc : AGAPType.values()) {
            if (rc.value == value) {
                enumIndex = rc.index;
                break;
            }
        ...
        if (enumIndex == Integer.MAX_VALUE) {
            return AGAPType.ENUM_UNINITIALIZED;
        }

        return AGAPType.values()[enumIndex];
    }

    public static AGAPType fromInt(int i) {
        return convert(i);
    }

...
//    BDStageBDChannelOnOnly = 5
    ...
    BubbleDetectMode(int enumIndex) {
     ...
     */
    public static BubbleDetectMode convert(int value) {
        int enumIndex = Integer.MAX_VALUE;

        for (BubbleDetectMode rc : BubbleDetectMode.values()) {
            if (rc.value == value) {
                enumIndex = rc.index;
                break;
            }
        ...
        if (enumIndex == Integer.MAX_VALUE) {
            return BubbleDetectMode.ENUM_UNINITIALIZED;
        }

        return BubbleDetectMode.values()[enumIndex];
    }

    public static BubbleDetectMode fromInt(int i) {
        return convert(i);
    }
